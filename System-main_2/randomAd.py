...
def preparation_files():
	...
	with open(SAVE_DIR+'\\advertising_time_stamp.csv','a',newline='') as f: # プログラム開始終了タイムスタンプ
		writer = csv.writer(f)
		writer.writerow(['start_quiz_system(perf)','start_program(perf)','start_program_date','finish_program(perf)','transision_finish_screen(perf)'])
...
# 閉じるボタンコマンド
def btn_click():
	global root, flag_click, time_close, is_disp
...
def ignore_btn_click():
	global root, flag_click, time_close, is_disp, ignore_rate, IGNORE_MAX
...
def display_ad(ad_kind, im_yoko_path, im_sikaku_path):
	...
	SIZE_YOKO, SIZE_SIKAKU, is_disp, img, img2, SIZE_ZENMEN
