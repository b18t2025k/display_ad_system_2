...
SIZE_YOKO = "700x150"
SIZE_SIKAKU = "300x240"
SIZE_ZENMEN = "360x420" # 前面系の白い部分含めた大きさ 画像wid:300なのでpaddign:60/2となる 上に余白は多くとる
...
POS_ZENMEN = "780+330" # (1920-360)/2=780 (1080-420)/2=330
...
def set_canvas():
	...
	root.roundedbutton.place(x=(360-198)/2,y=240+80)
...
def ignore_btn_click():
	global root, flag_click, time_close, is_disp, ignore_num, IGNORE_MAX, root2
	print("flag_click") # デバッグ用
	if 1 < ignore_num:
...
def ad_log(time, ad_kind, is_result, is_correct, is_disp, content, content2=""):
	...
	if is_disp == True:
		...
		if ad_kind != 0:
			...
			if ad_kind == 1 or ad_kind == 2: # ログ出力時に上下の2つ目が出なくなるのでログにも出ないように修正
				contents[1] = ""
...
def set_display_ad(ad_kind):
	...
	if ad_kind == 1 or ad_kind == 2: # 上下 ← 上下から下のみに変更
		...
		root.roundedbutton.place(x=(360-198)/2,y=240+80) # 閉じるボタン:198x59(高さは適当)
		pos = POS_ZENMEN
		pos2 = "0+0"
		geo = SIZE_ZENMEN + "+" + pos
		...
		canvas.create_line(0+2, 0+2, 0+2, 420+2, fill='gray', width = 4) # 時間あれば変数使う
		canvas.create_line(0+2, 420+2, 360+2, 420+2, fill='gray', width = 4)
		canvas.create_line(360+2, 420+2, 360+2, 0+2, fill='gray', width = 4)
		canvas.create_line(360+2, 0+2, 0+2, 0+2, fill='gray', width = 4)
	...
	if ad_kind == 1 or ad_kind == 2:
		...
		item = canvas.create_image(0+2+30, 0+2+30+4/2+20, image=img, anchor=tkinter.NW)
		item2 = canvas2.create_image(0, 0, image=img2, anchor=tkinter.NW)
